...
if args.attack:
    if args.kernel_size > 0:
        result_dir  = os.path.join(args.result_dir, dataset, 'corrected',  'trans_{}_para_trans_{}_flc_{}_adv_train_{}'.format(args.kernel_size, args.para_kernel_size, args.flc, args.adv_trained), args.method, 'iterations_{}'.format(args.iterations), 'alpha_{}'.format(args.alpha), 'epsilon_{}'.format(args.epsilon))
    else:    
        result_dir  = os.path.join(args.result_dir, dataset, 'corrected', 'flc_{}_alpha_{}_blur_{}_alpha_{}_drop_alpha_testing_{}_drop_alpha_adv_train_{}'.format('without' if not args.use_alpha else 'with', 'with' if args.blur else 'without', 'learned' if args.learn_alpha else 'random', drop_word, "without" if args.test_wo_drop_alpha else "with", args.adv_trained), args.method, 'iterations_{}'.format(args.iterations), 'alpha_{}'.format(args.alpha), 'epsilon_{}'.format(args.epsilon))
else:
    if args.kernel_size > 0:
        result_dir  = os.path.join(args.result_dir, dataset, 'corrected', 'no_attack', 'trans_{}_para_trans_{}_flc_{}_adv_train_{}'.format(args.kernel_size, args.para_kernel_size, args.flc, args.adv_trained))
    else:    
        result_dir  = os.path.join(args.result_dir, dataset, 'corrected', 'no_attack', 'flc_{}_alpha_{}_blur_{}_alpha_{}_drop_alpha_testing_{}_drop_alpha_adv_train_{}'.format('without' if not args.use_alpha else 'with', 'with' if args.blur else 'without', 'learned' if args.learn_alpha else 'random', drop_word, "without" if args.test_wo_drop_alpha else "with", args.adv_trained))
