...
def save_images(images, adv=False, notes='', iteration=1):
    counter = 0
    for image in images:
        image = T.ToPILImage()(image)
        image_name = "/home/agnihotri/projects/Restormer/Restormer/adv_images/iteration_{}/image_{}{}{}.png".format(iteration, counter, '_adv' if adv else '', notes)
        image.save(image_name)
        counter += 1

def hist_plot(gradients, adv=False, notes='', iteration=1):
    gradients = gradients.clone().detach().cpu().numpy()
    plt.hist(gradients)
    plt.savefig("/home/agnihotri/projects/Restormer/Restormer/adv_images/iteration_{}/weights_{}{}.png".format(iteration, 'adv' if adv else 'clean', notes))
    plt.clf()